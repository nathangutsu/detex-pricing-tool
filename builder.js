...
function buildAssembledSku(raw) {
  ...
  if (!seriesKey) {
    return {
      error: `Series "${seriesToken}" isn't recognized, or isn't supported yet by the Device Builder. The Device Builder covers exit devices ${Object.keys(SERIES_RULES).join(', ')} and the exit control locks ${Object.keys(ECL_RULES.models).join(', ')}. Try the Lookup tab if this is meant to be a single flat part number instead.`
    };
  }
...
// ====================================================================================================
// Exit control locks: ECL-230D, ECL-230D-PH, ECL-600 and the ECL-230X family (SP, TD, TB, TDB, and -W).
// Data: ecl.js (PDF pp.85-94, generated by tools/gen_ecl.py). Base price = the spreadsheet row (live).
// ====================================================================================================
const ECL_MODEL_KEYS = Object.keys(ECL_RULES.models).sort((a, b) => b.length - a.length);
const ECL_ACC_INDEX = {};   // upper-case code -> accessory entry (also reachable without the "ECL-" prefix)
Object.keys(ECL_RULES.accessories).forEach(code => {
  const key = code.toUpperCase();
  ECL_ACC_INDEX[key] = Object.assign({ code }, ECL_RULES.accessories[code]);
  if (key.startsWith('ECL-') && !ECL_ACC_INDEX[key.slice(4)]) ECL_ACC_INDEX[key.slice(4)] = ECL_ACC_INDEX[key];
});
const ECL_DASHED = new Set(['VRA-143B', 'VRA-143C', 'VRA-143D']);
Object.keys(ECL_ACC_INDEX).forEach(k => { if (k.includes('-')) ECL_DASHED.add(k); });
Object.keys(ECL_RULES.families).forEach(f => Object.keys(ECL_RULES.families[f]).forEach(k => { if (k.includes('-')) ECL_DASHED.add(k.toUpperCase()); }));

function mergeDashedTokens(tokens, dashedSet) {
  const out = [];
  let i = 0;
  while (i < tokens.length) {
    let matched = null;
    for (let span = 3; span >= 2 && !matched; span--) {
      if (i + span > tokens.length) continue;
      const joined = tokens.slice(i, i + span).join('-').toUpperCase();
      if (dashedSet.has(joined)) matched = { span, joined };
    }
    if (matched) { out.push(matched.joined); i += matched.span; }
    else { out.push(tokens[i].toUpperCase()); i += 1; }
  }
  return out;
}

function buildEclSku(input) {
  const model = ECL_MODEL_KEYS.find(m => input === m || input.startsWith(m + '-'));
  if (!model) return null;
  const mdef = ECL_RULES.models[model];
  const famOpts = ECL_RULES.families[mdef.family] || null;   // ECL-230X options; null for 230D/600
  const baseRec = DETEX_DATA.find(r => r.part === model && r.cat === 'EXIT CONTROL LOCKS');
  const lines = [];
  const unresolved = [];
  if (!baseRec) {
    return { partNumber: input, seriesKey: model, seriesLabel: `Exit Control Lock ${model}`, lines, total: 0,
      unresolved: [`${model} has no priced row in the current data — do not assume a price.`] };
  }
  lines.push({ label: `${model} — ${mdef.label}`, price: baseRec.price, page: mdef.page });

  const rest = input.slice(model.length).replace(/^-+/, '');
  const tokens = mergeDashedTokens(rest ? rest.split(/-+/).filter(Boolean) : [], ECL_DASHED);

  let black = false, hurricane = false, sn1 = false, heightToken = null, cylinder = false, needsCyl = null;
  const dxLines = [];
  let vraType = null;
  const cyl = ECL_RULES.cylinder;
  const otherFamilyHas = code => Object.keys(ECL_RULES.families).some(f => ECL_RULES.families[f][code]);

  tokens.forEach(t => {
    if (t === '36' || t === '48') return;   // 36"-48" is the standard door width
    if (t === 'BLK' || t === 'BK') { black = true; return; }
    if (cyl.codes[t]) {
      cylinder = true;
      lines.push({ label: `${t} — ${cyl.codes[t]}, factory installed`, price: cyl.price, page: cyl.page });
      return;
    }
    if (t === 'MC65' || t === 'IC7') {
      unresolved.push(`"${t}" is a mortise cylinder; ECL locks take rim cylinders (RC65 or IC7R, PDF p.95). Not priced.`);
      return;
    }
    const vm = t.match(/^VRA-143([BCD])$/);
    if (vm) {
      if (model !== 'ECL-230D') unresolved.push(`"${t}" vertical rod assemblies are for the ECL-230D only (PDF p.88), not ${model}. Not priced.`);
      else vraType = vm[1];
      return;
    }
    if (famOpts) {
      const o = famOpts[t];
      if (o) {
        if (t === 'H') { hurricane = true; lines.push({ label: `H — ${o.label}`, price: o.price, page: o.page }); return; }
        if (t === 'SN1') { sn1 = o; return; }
        if (o.onlyModels && !o.onlyModels.includes(model)) {
          unresolved.push(`"${t}" (${o.label}) is TB only per the catalog (PDF p.${o.page}), not ${model}. Not priced.`);
          return;
        }
        if (o.black !== undefined) { dxLines.push({ t, o }); return; }
        if (t === '84' || t === '96' || t === '120') heightToken = t;
        if (o.standardNoCharge) {
          lines.push({ label: `${t} — ${o.label} (standard on ${model.includes('TB') ? 'TB' : 'this model'}, no charge)`, price: 0, page: o.page });
          return;
        }
        lines.push({ label: `${t} — ${o.label}${o.price === 0 ? ' (no charge)' : ''}`, price: o.price, page: o.page });
        return;
      }
      const tr = ECL_RULES.xTrims[t];
      if (tr) {
        if (t === '03T' || t === '03PP') needsCyl = t;
        lines.push({ label: `${t} — ${tr.label}`, price: tr.price, page: tr.page });
        return;
      }
    }
    const acc = ECL_ACC_INDEX[t];
    if (acc) {
      if (!acc.models.includes(model)) {
        if (acc.code === 'ECL-498K' && model === 'ECL-600') {
          unresolved.push(`"${t}" (Outside Pull and Warning Sign) is not for use on fire rated doors, and the ECL-600 is fire rated (PDF p.86). Not priced.`);
        } else {
          unresolved.push(`"${t}" (${acc.label}) isn't offered with ${model} — it fits ${acc.models.join(', ')} (PDF p.${acc.page}). Not priced.`);
        }
        return;
      }
      lines.push({ label: `${acc.code} — ${acc.label}`, price: acc.price, page: acc.page });
      return;
    }
    if (model === 'ECL-230D' && (t === '84' || t === '96' || t === '120')) { heightToken = t; return; }
    if (!famOpts && (t === '84' || t === '96' || t === '120')) {
      unresolved.push(`"${t}" is a door height and only applies to vertical rod assemblies (VRA-143) on the ${model}. Not priced.`);
      return;
    }
    if (!famOpts && otherFamilyHas(t)) {
      unresolved.push(`"${t}" is an ECL-230X option; it isn't offered with ${model}. Not priced.`);
      return;
    }
    if (famOpts && otherFamilyHas(t)) {
      unresolved.push(`"${t}" isn't offered with ${model} (PDF p.${mdef.page}). Not priced.`);
      return;
    }
    unresolved.push(`"${t}" didn't match an option, trim, or accessory for ${model}. Not priced — verify manually.`);
  });

  dxLines.forEach(({ t, o }) => {
    lines.push({
      label: `${t} — ${o.label}${black ? ' (black)' : ' (gray)'}`,
      price: black ? o.black : o.price,
      page: 92,
    });
  });
  if (sn1) {
    lines.push(hurricane
      ? { label: `SN1 — Sex Nut Kits (${sn1.kits} required) — included with Hurricane Rated, no charge`, price: 0, page: sn1.page }
      : { label: `SN1 — Sex Nut Kits (${sn1.kits} kits required x $${sn1.price})`, price: sn1.kits * sn1.price, page: sn1.page });
  }
  if (black) lines.push({ label: 'BLK — Black finish (lock body: no additional charge)', price: 0, page: mdef.page });
  if (vraType) {
    const tier = heightToken || '84';
    const price = ECL_RULES.vra[vraType][tier];
    if (price === undefined) {
      unresolved.push(`VRA-143${vraType} isn't offered for a ${tier}" door height (PDF p.88: type ${vraType} goes up to ${Object.keys(ECL_RULES.vra[vraType]).join('/')}"). Not priced.`);
    } else {
      const tierLabel = tier === '84' ? 'standard 84"' : (tier === '96' ? 'special height over 84" to 96"' : 'over 96" to 120"');
      lines.push({ label: `VRA-143${vraType} — Vertical Rod Assembly, installation type ${vraType}, ${tierLabel}`, price, page: 88 });
    }
  } else if (model === 'ECL-230D' && heightToken) {
    unresolved.push(`"${heightToken}" is a door height for vertical rod assemblies; no VRA-143 is in this string. Not priced.`);
  }

  if (!cylinder) {
    if (!famOpts) {
      unresolved.push(`${model} ships less its rim cylinder (required, PDF p.94) — none specified, so no cylinder cost is included. Add RC65 or IC7R ($${cyl.price} installed).`);
    } else if (needsCyl) {
      unresolved.push(`${needsCyl} has outside key control and needs a rim cylinder — none specified, so no cylinder cost is included. Add RC65 or IC7R ($${cyl.price} installed).`);
    }
  }

  const total = lines.reduce((s, l) => s + (l.price || 0), 0);
  return { partNumber: input, seriesKey: model, seriesLabel: `Exit Control Lock ${model}`, lines, unresolved, total };
}
